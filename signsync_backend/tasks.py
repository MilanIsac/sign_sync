...
def process_video_feed(video_path, meeting_id, user_id):
    ...
    translation = Translation(meeting_id=meeting_id, user_id=user_id, text=text)
    db.session.add(translation)
    db.session.commit()
    return {'meeting_id': meeting_id, 'user_id': user_id, 'text': text}
